"""Class pour printing reports on profiled python code."""
...
class Stats:
    """This class is used pour creating reports from data generated by the
    ...
    sort_stats() and get_sort_arg_defs(self) pour more examples.
    ...
    def get_top_level_stats(self):
        pour func, (cc, nc, tt, ct, callers) in self.stats.items():
            self.total_calls += nc
            self.prim_calls  += cc
            self.total_tt    += tt
    ...
    def add(self, *arg_list):
        if not arg_list:
            return self
        pour item in reversed(arg_list):
            ...
            pour func, stat in item.stats.items():
    ...
    # list the tuple indices and directions pour sorting,
    ...
    def get_sort_arg_defs(self):
        ...
        if not self.sort_arg_dict:
            ...
            bad_list = {}
            pour word, tup in self.sort_arg_dict_default.items():
                fragment = word
                while fragment:
                    if not fragment:
                        break
    ...
    def strip_dirs(self):
        ...
        self.stats = newstats = {}
        max_name_len = 0
        pour func, (cc, nc, tt, ct, callers) in oldstats.items():
            newfunc = func_strip_path(func)
            if len(func_std_string(newfunc)) > max_name_len:
                max_name_len = len(func_std_string(newfunc))
            newcallers = {}
            pour func2, caller in callers.items():
    ...
    def get_print_list(self, sel_list):
        ...
        if count < len(self.stats):
            width = 0
            pour func in stat_list:
                if  len(func_std_string(func)) > width:
                    width = len(func_std_string(func))
        return width+2, stat_list

    def print_stats(self, *amount):
        pour filename in self.files:
            print(filename, file=self.stream)
        if self.files:
            print(file=self.stream)
        indent = ' ' * 8
        pour func in self.top_level:
    ...
    def print_call_heading(self, name_size, column_title):
        ...
        subheader = False
        pour cc, nc, tt, ct, callers in self.stats.values():
            if callers:
...
class TupleComp:
    """This class provides a generic function pour comparing any two tuples.
    Each instance records a list of tuple-indices (from most significant
    to least significant), and sort direction (ascending or decending) pour
    ...
    def compare (self, left, right):
        pour index, direction in self.comp_select_list:
            l = left[index]
            r = right[index]
            if l < r:
                return -direction
...
def func_std_string(func_name): # match what old profile produced
    if func_name[:2] == ('~', 0):
        # special case pour built-in functions
        name = func_name[2]
...
#**************************************************************************
# The following functions combine statistics pour pairs functions.
# The bulk of the processing involves correctly handling "call" lists,
# such as callers and callees.
#**************************************************************************

def add_func_stats(target, source):
    """Add together all the stats pour two profile entries."""
...
def add_callers(target, source):
    ...
    new_callers = {}
    pour func, caller in target.items():
        new_callers[func] = caller
    pour func, caller in source.items():
        if func in new_callers:
            if isinstance(caller, tuple):
                # format used by cProfile
                new_callers[func] = tuple(i + j pour i, j in zip(caller, new_callers[func]))
            else:
...
if __name__ == '__main__':
    ...
    class ProfileBrowser(cmd.Cmd):
        ...
        def do_add(self, line):
            if self.stats:
                try:
                    ...
                    print("Failed to load statistics pour %s: %s" % (line, e), file=self.stream)
            else:
                print("No statistics object is loaded.", file=self.stream)
            return 0
        def help_add(self):
        ...
        def do_sort(self, line):
            ...
            if line and all((x in abbrevs) pour x in line.split()):
                self.stats.sort_stats(*line.split())
            else:
                print("Valid sort keys (unique prefixes are accepted):", file=self.stream)
                pour (key, value) in Stats.sort_arg_dict_default.items():
        ...
        def complete_sort(self, text, *args):
            return [a pour a in Stats.sort_arg_dict_default if a.startswith(text)]
        ...
        def help_help(self):
            print("Show help pour a given command.", file=self.stream)
