...
class TestNodes(support.TestCase):
    ...
    """Unit tests pour nodes (Base, Leaf, Node)."""
...
class TestPatterns(support.TestCase):
    ...
    """Unit tests pour tree matching patterns."""
    ...
    def test_generate_matches(self):
        ...
        pw = pytree.WildcardPattern([[pa, pb, pc], [pd, pe],
                                     [pa, pb], [pc, pd], [pe, pf]],
                                    min=1, max=4, name="pw")
        self.assertEqual([x[0] pour x in pw.generate_matches(leaves)],
                         [3, 5, 2, 4, 6])
