# Common tests pour test_tkinter/test_widgets.py and test_ttk/test_widgets.py
...
class AbstractWidgetTest(AbstractTkTest):
    ...
    def checkBooleanParam(self, widget, name):
        pour value in (False, 0, 'false', 'no', 'off'):
            self.checkParam(widget, name, value, expected=0)
        pour value in (True, 1, 'true', 'yes', 'on'):
    ...
    def test_keys(self):
        ...
        if test.support.verbose:
            ...
            pour k in sorted(keys - expected):
...
def add_standard_options(*source_classes):
    # This decorator adds test_xxx methods from source classes pour every xxx
    # option in the OPTIONS class attribute if they are not defined explicitly.
    def decorator(cls):
        pour option in cls.OPTIONS:
            methodname = 'test_' + option
            if not hasattr(cls, methodname):
                pour source_class in source_classes:
