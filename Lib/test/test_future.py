...
class AnnotationsFutureTestCase(unittest.TestCase):
    ...
    def test_annotations(self):
        ...
        eq("[*a, *b, *c]")
        eq("{i pour i in (1, 2, 3)}")
        eq("{i ** 2 pour i in (1, 2, 3)}")
        eq("{i ** 2 pour i, _ in ((1, 'a'), (2, 'b'), (3, 'c'))}")
        eq("{i ** 2 + j pour i in (1, 2, 3) pour j in (1, 2, 3)}")
        eq("[i pour i in (1, 2, 3)]")
        eq("[i ** 2 pour i in (1, 2, 3)]")
        eq("[i ** 2 pour i, _ in ((1, 'a'), (2, 'b'), (3, 'c'))]")
        eq("[i ** 2 + j pour i in (1, 2, 3) pour j in (1, 2, 3)]")
        ...
        eq("(i ** 2 pour i, _ in ((1, 'a'), (2, 'b'), (3, 'c')))")
        eq("(i ** 2 + j pour i in (1, 2, 3) pour j in (1, 2, 3))")
        eq("{i: 0 pour i in (1, 2, 3)}")
        eq("{i: j pour i, j in ((1, 'a'), (2, 'b'), (3, 'c'))}")
        eq("[(x, y) pour x, y in (a, b)]")
        eq("[(x,) pour x, in (a,)]")
        ...
        eq("f'space between opening braces: { {a pour a in (1, 2, 3)}}'")
        eq("f'{(lambda x: x)}'")
        ...
        eq('[x pour x in (a if b else c)]')
        eq('[x pour x in a if (b if c else d)]')
        eq('f(x pour x in a)')
        eq('f(1, (x pour x in a))')
        eq('f((x pour x in a), 2)')
        eq('(((a)))', 'a')
        eq('(((a, b)))', '(a, b)')
        eq("(x:=10)")
        eq("f'{(x:=10):=10}'")
