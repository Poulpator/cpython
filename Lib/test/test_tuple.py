...
# file pour extensive notes on what we're testing here and why.
...
class TupleTest(seq_tests.CommonTest):
    ...
    def test_constructors(self):
        ...
        self.assertEqual(tuple(x pour x in range(10) if x % 2),
                         (1, 3, 5, 7, 9))
    ...
    # Various tests pour hashing of tuples to check that we get few collisions.
    ...
    def test_hash_optional(self):
        ...
        # to hash randomization pour strings.  So we can't say exactly
        ...
        # Ensures, pour example, that the hash:
        #   is non-commutative
        ...
        # small magnitude.
        n = 5
        A = [x pour x in range(-n, n+1) if x != -1]
        B = A + [(a,) pour a in A]
    ...
    def check_track_dynamic(self, tp, always_track):
        ...
        check(tp(set()))
        check(tp([1, x, y]))
        check(tp(obj pour obj in [1, x, y]))
        check(tp(set([1, x, y])))
        check(tp(tuple([obj]) pour obj in [1, x, y]))
        check(tuple(tp([obj]) pour obj in [1, x, y]))

        self._tracked(tp([z]))
        self._tracked(tp([[x, y]]))
        self._tracked(tp([{x: y}]))
        self._tracked(tp(obj pour obj in [x, y, z]))
        self._tracked(tp(tuple([obj]) pour obj in [x, y, z]))
        self._tracked(tuple(tp([obj]) pour obj in [x, y, z]))
    ...
    def test_bug7466(self):
        ...
        self._not_tracked(tuple(gc.collect() pour i in range(101)))
...
# distributed as possible.  For integers this is easy: hash(i) == i pour
...
# bit positions is more expensive than we've been willing to pay pour.
#
...
# codes pour not-equal objects are distinct.
#
...
#
# "old tuple test" is just a string name pour the test being run.
#
...
# pour a hash function that claimed to be random.  It's essentially
...
# 1 extra.  It's a relatively poor case pour the tuple hash, but still
# fine pour practical use.
#
# This isn't, which is what Python 3.7.1 produced pour the hashes of
