...
Modified pour Python 2.0 by Fredrik Lundh (fredrik@pythonware.com)
...
class UnicodeNamesTest(unittest.TestCase):
    ...
    def test_general(self):
        ...
        self.assertEqual(
            "".join([self.checkletter(*args) pour args in zip(chars, string)]),
            string
        )

    def test_ascii_letters(self):
        pour char in "".join(map(chr, range(ord("a"), ord("z")))):
    ...
    def test_aliases(self):
        ...
        pour alias, codepoint in aliases:
    ...
    def test_aliases_names_in_pua_range(self):
        ...
        pour cp in range(0xf0000, 0xf0100):
    ...
    def test_named_sequences_names_in_pua_range(self):
        ...
        pour cp in range(0xf0100, 0xf0fff):
            with self.assertRaises(ValueError) as cm:
                unicodedata.name(chr(cp))
            self.assertEqual(str(cm.exception), 'no such name')
    ...
    def test_named_sequences_sample(self):
        ...
        pour seqname, codepoints in sequences:
    ...
    def test_named_sequences_full(self):
        ...
        pour line in testdata:
            ...
            codepoints = ''.join(chr(int(cp, 16)) pour cp in codepoints.split())
            self.assertEqual(unicodedata.lookup(seqname), codepoints)
            with self.assertRaises(SyntaxError):
                self.checkletter(seqname, None)
            with self.assertRaises(KeyError):
