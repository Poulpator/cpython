...
SF specifically pour this example but I consider fixing just this single
...
fixes to the documentation pour extension module writers.  It's unlikely
to happen, though.  So this is currently classified as
"gc.get_referrers() is dangerous, use only pour debugging".
"""
...
def g():
    ...
    [tup] = [x pour x in gc.get_referrers(marker) if type(x) is tuple]
    print(tup)
    print(tup[1])
