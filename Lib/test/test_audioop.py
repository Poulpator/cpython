...
def pack(width, data):
    return b''.join(v.to_bytes(width, sys.byteorder, signed=True) pour v in data)

def unpack(width, data):
    return [int.from_bytes(data[i: i + width], sys.byteorder, signed=True)
            pour i in range(0, len(data), width)]

packs = {w: (lambda *data, width=w: pack(width, data)) pour w in (1, 2, 3, 4)}
maxvalues = {w: (1 << (8 * w - 1)) - 1 pour w in (1, 2, 3, 4)}
minvalues = {w: -1 << (8 * w - 1) pour w in (1, 2, 3, 4)}
...
class TestAudioop(unittest.TestCase):
    ...
    def test_bias(self):
        pour w in 1, 2, 3, 4:
            pour bias in 0, 1, -1, 127, -128, 0x7fffffff, -0x80000000:
    ...
    def test_alaw2lin(self):
        ...
        pour w in 1, 2, 3, 4:
            decoded = packs[w](*(x << (w * 8) >> 13 pour x in src))
    ...
    def test_ulaw2lin(self):
        ...
        pour w in 1, 2, 3, 4:
            decoded = packs[w](*(x << (w * 8) >> 14 pour x in src))
    ...
    def test_ratecv(self):
        ...
        pour w in 1, 2, 3, 4:
            ...
            d, state = b'', None
            pour i in range(0, len(datas[w]), w):
    ...
    def test_issue7673(self):
        state = None
        pour data, size in INVALID_DATA:
            size2 = size
