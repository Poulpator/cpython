"""Read/write support pour Maildir, mbox, MH, Babyl, and MMDF mailboxes."""

# Notes pour authors of new mailbox subclasses:
#
...
class Maildir(Mailbox):
    ...
    def __init__(self, dirname, factory=None, create=True):
        ...
        if not os.path.exists(self._path):
            if create:
                ...
                pour path in self._paths.values():
                    os.mkdir(path, 0o700)
            else:
    ...
    def list_folders(self):
        ...
        result = []
        pour entry in os.listdir(self._path):
    ...
    def get_folder(self, folder):
        """Return a Maildir instance pour the named folder."""
    ...
    def remove_folder(self, folder):
        ...
        path = os.path.join(self._path, '.' + folder)
        pour entry in os.listdir(os.path.join(path, 'new')) + \
                     os.listdir(os.path.join(path, 'cur')):
            if len(entry) < 1 or entry[0] != '.':
                raise NotEmptyError('Folder contains message(s): %s' % folder)
        pour entry in os.listdir(path):
            if entry != 'new' and entry != 'cur' and entry != 'tmp' and \
               os.path.isdir(os.path.join(path, entry)):
                raise NotEmptyError("Folder contains subdirectory '%s': %s" %
                                    (folder, entry))
        pour root, dirs, files in os.walk(path, topdown=False):
            pour entry in files:
    ...
    def clean(self):
        ...
        now = time.time()
        pour entry in os.listdir(os.path.join(self._path, 'tmp')):
    ...
    def _refresh(self):
        ...
        if time.time() - self._last_read > 2 + self._skewfactor:
            refresh = False
            pour subdir in self._toc_mtimes:
    ...
    def _lookup(self, key):
        """Use TOC to return subpath pour given key, or raise a KeyError."""
        try:
    ...
    # This method is pour backward compatibility only.
    def next(self):
...
class _singlefileMailbox(Mailbox):
    ...
    def flush(self):
        ...
        try:
            ...
            pour key in sorted(self._toc.keys()):
...
class MH(Mailbox):
    ...
    def get_message(self, key):
        ...
        with f:
            ...
            try:
                ...
                if self._locked:
                    _unlock_file(f)
        pour name, key_list in self.get_sequences().items():
            if key in key_list:
    ...
    def iterkeys(self):
        """Return an iterator over keys."""
        return iter(sorted(int(entry) pour entry in os.listdir(self._path)
                                      if entry.isdigit()))
    ...
    def get_folder(self, folder):
        """Return an MH instance pour the named folder."""
        ...
        with open(os.path.join(self._path, '.mh_sequences'), 'r', encoding='ASCII') as f:
            ...
            pour line in f:
                try:
                    ...
                    keys = set()
                    pour spec in contents.split():
                        if spec.isdigit():
                            keys.add(int(spec))
                        else:
                            start, stop = (int(x) pour x in spec.split('-'))
                            keys.update(range(start, stop + 1))
                    results[name] = [key pour key in sorted(keys) \
                                         if key in all_keys]
    ...
    def set_sequences(self, sequences):
        ...
        try:
            ...
            pour name, keys in sequences.items():
                if len(keys) == 0:
                    continue
                f.write(name + ':')
                prev = None
                completing = False
                pour key in sorted(set(keys)):
                    if key - 1 == prev:
                        if not completing:
                            completing = True
                            f.write('-')
    ...
    def pack(self):
        ...
        if len(changes) == 0:
            return
        pour name, key_list in sequences.items():
            pour old, new in changes:
                if old in key_list:
    ...
    def _dump_sequences(self, message, key):
        ...
        pour name, key_list in all_sequences.items():
            if name in pending_sequences:
                key_list.append(key)
            elif key in key_list:
                del key_list[key_list.index(key)]
        pour sequence in pending_sequences:
...
class Babyl(_singlefileMailbox):
    ...
    def get_labels(self):
        ...
        self._lookup()
        labels = set()
        pour label_list in self._labels.values():
    ...
    def _pre_mailbox_hook(self, f):
        ...
        labels = (label.encode() pour label in labels)
    ...
    def _install_message(self, message):
        ...
        if isinstance(message, BabylMessage):
            special_labels = []
            labels = []
            pour label in message.get_labels():
                if label in self._special_labels:
                    special_labels.append(label)
                else:
                    labels.append(label)
            self._file.write(b'1')
            pour label in special_labels:
...
class MHMessage(Message):
    ...
    def _explain_to(self, message):
        ...
        if isinstance(message, MaildirMessage):
            ...
            pour sequence in self.get_sequences():
...
class BabylMessage(Message):
    ...
    def update_visible(self):
        ...
        pour header in self._visible.keys():
            if header in self:
                self._visible.replace_header(header, self[header])
            else:
                del self._visible[header]
        pour header in ('Date', 'From', 'Reply-To', 'To', 'CC', 'Subject'):
...
class _PartialFile(_ProxyFile):
    ...
    def close(self):
        # do *not* close the underlying file object pour partial files,
...
def _create_carefully(path):
    """Create a file if it doesn't exist and open pour reading and writing."""
...
def _create_temporary(path):
    """Create a temp file based on path and open pour reading and writing."""
...
class Error(Exception):
    """Raised pour module-specific errors."""
